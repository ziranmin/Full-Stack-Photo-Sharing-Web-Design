######################################
# Author Ziran Min <minziran@bu.edu> 
######################################
...
app.config['MYSQL_DATABASE_PASSWORD'] = 'xxxxxx' #CHANGE THIS TO YOUR MYSQL PASSWORD
...
def getAllFriends():
	uid = getUserIdFromEmail(flask_login.current_user.id)
	cursor = conn.cursor()
	cursor.execute("SELECT U.first, U.last, U.birth, U.hometown, U.gender, U.uid FROM Friends F, Users U WHERE F.uid='{0}' AND F.f_uid=U.uid".format(uid))
	return cursor.fetchall()
...
def getUserFirstNameFromEmail(email):
	cursor = conn.cursor()
	cursor.execute("SELECT first  FROM Users WHERE email = '{0}'".format(email))
	return cursor.fetchone()[0]

def getUserLastNameFromEmail(email):
	cursor = conn.cursor()
	cursor.execute("SELECT last  FROM Users WHERE email = '{0}'".format(email))
	return cursor.fetchone()[0]
...
def getAlbumTopPhotoID(aid):
	cursor = conn.cursor()
	cursor.execute("SELECT MAX(pid)  FROM Album WHERE aid = '{0}'".format(aid))
	return cursor.fetchone()[0]

def getPhoto(aid, pid):
	cursor = conn.cursor()
	cursor.execute("SELECT data  FROM Photo WHERE aid = '{0}' and pid ='{1}'".format(aid, pid) )
	return cursor.fetchone()[0]

def updateAlbumCover(aid):
	cursor = conn.cursor()
	pid = getAlbumTopPhoto(aid)
	data =  getPhoto(aid, pid)
	cursor.execute("UPDATE Album SET cover='{0}' WHERE aid='{1}' AND pid='{2}'".format(data, aid, pid))
'''
def deletePhoto(pid):
	cursor = conn.cursor()
...
def deleteTags(pid, tags):
	cursor = conn.cursor()
	for tag in tags.split(', '):
		cursor.execute("DELETE FROM Tag where pid = '{0}' AND word = '{1}'".format(pid, tag))
	conn.commit()
...
def handleComments(content, pid):
	...
	if flask_login.current_user.is_authenticated:
		uid = getUserIdFromEmail(flask_login.current_user.id)
		authorName = getUserFirstNameFromEmail(flask_login.current_user.id) + "  " + getUserLastNameFromEmail(flask_login.current_user.id)
		if getUserIDByPhotoID(pid) != uid:
...
def getUserAllTags(uid):
	cursor = conn.cursor() 
	cursor.execute("SELECT DISTINCT word FROM Tag WHERE uid = '{0}'".format(uid))
	return cursor.fetchall()
	

def getTagRecom(uid):
	cursor = conn.cursor() 
	cursor.execute("Select T1.word, COUNT(*) FROM (Select Distinct T0.pid From Tag T0 where T0.word NOT IN (Select Distinct T.word From Tag T where T.uid = '{0}') and uid <> '{1}') A, Tag T1 WHERE T1.pid = A.pid AND T1.word NOT IN (Select Distinct T2.word From Tag T2 where T2.uid = '{2}') GROUP BY T1.word ORDER BY COUNT(*) DESC LIMIT 10".format(uid, uid, uid))
	return cursor.fetchall()
...
def protected():
	...
	if request.method == 'POST':
		...
		# delete photo
			...
			deletePhoto(pid)
			#aid = getAlbumID(pid)
			#updateAlbumCover(aid)
		# delete album
		elif request.form.get('deleteAlbum') is not None:
			aname = request.form.get('deleteAlbum')
			deleteAlbum(aname)
		# add tags	
		elif request.form.get('addtags') is not None:
			tags_to_add = request.form.get('addtags')
			pid = request.form.get('photoIDtoAddTags')
			addTags(pid, tags_to_add)
		# delete tags	
		elif request.form.get('deletetags') is not None:
			tags_to_delete = request.form.get('deletetags')
			pid = request.form.get('photoIDtoDeleteTags')
			deleteTags(pid, tags_to_delete)
		# upload photo
		else:
...
def friend():
	if request.method == 'POST':
		...
		conn.commit()
	return render_template('friend.html', strangers=getAllStrangers(), friends=getAllFriends(), userInfo = getUserInfo()) 
...
def account():
	uid = getUserIdFromEmail(flask_login.current_user.id)
	return render_template('account.html', userInfo=getUserInfo(), allphotos=getAllPhotos(),allcomments=getAllComments(), alllikes=getAllLikes(), tags=getAllTags(), usertags = getUserAllTags(uid))
...
def register_user():
	try:
		...
		#profdata = base64.standard_b64encode(profilepic.read())
	except:
	...
	if test:
		...
		elif request.form.get('prof') == "yes":
			profilepic = request.files['profile']
			profdata = base64.standard_b64encode(profilepic.read())
			print cursor.execute("INSERT INTO Users (email, password, first, last, birth, hometown, gender, profpic) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}','{7}')".format(email, password, first, last, birth, hometown, gender, profdata))
		conn.commit()
		user = User()
		user.id = email
